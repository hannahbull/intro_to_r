...
heads <- get_heads_tab %>%
  ...
  subset(State!="Switzerland" | (State=="Switzerland" & leader=="Ueli Maurer")) %>%
...
heads <- heads %>%
  mutate(political_party=ifelse(leader=="Xi Jinping", "Communist Party of China", political_party))
```
...
heads <- heads %>%
  ...
  mutate(political_position_clean=gsub("currently|current|.*present|formerly.*|[0-9].*|\\,.*", "", political_position_clean)) %>%
  ...
  mutate(political_position_clean=gsub("catch all", "centre/big tent", political_position_clean)) %>%
  mutate(political_position_clean=gsub("centre left to centre", "centre to centre left", political_position_clean)) %>%
  ...
  mutate(political_position_clean=factor(political_position_clean,
                                         levels =c("far left", "left wing to far left" ,"left wing","centre left to left wing", "centre left", 
                                                   "centre to centre left", "centre/big tent", "centre to centre right", "centre right",
                                                   "centre right to right wing", "right wing", "right wing to far right", "far right", "unavailable"),
                                         labels =c("far left","left wing to far left", "left wing","centre left to left wing", "centre left", 
                                                   "centre to centre left", "centre/big tent", "centre to centre right", "centre right",
                                                   "centre right to right wing", "right wing", "right wing to far right", "far right", "unavailable"))) %>%
...
plot <- ggplot() +
  ...
  scale_fill_manual(values = c("#FF0000","#E90015","#D4002A", "#BF003F", "#AA0055", 
                               "#94006A", "#7F007F", "#6A0094","#5500AA", "#3F00BF", 
                               "#2A00D4","#1500E9", "#0000FF", "#bebebe")) + coord_fixed()
...
Upload your script at the link provided on the homepage.