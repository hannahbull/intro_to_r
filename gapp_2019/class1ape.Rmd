...
arrd <- readRDS("Output/arrd.rds") %>% select(Arrondissement=COM, P13_POP0014:P13_POP75P) %>% 
...
There are many different libraries in the R universe for a wide variety of tasks. In this first class, we will be covering methods to import and transform data in R using the `tidyverse`. I try to adhere to the [tidyverse style guide](https://style.tidyverse.org/). 
...
## Elements of R-Studio

![R-Studio interface](Data/images/rstudio.png)
...
All the data for today's exercise can be downloaded from [here](https://drive.google.com/open?id=115sQqs8LVqQmsKYRORTIAu3KZvUHczh0). 

This folder contains the French population data at the IRIS level (50k units in metropolitan France) [(source: Insee website)](https://www.insee.fr/fr/statistiques/2386737), the shapefiles for the IRIS [(source)](http://professionnels.ign.fr/contoursiris) and the shapefiles for the arrondissements in Paris [(source)](https://opendata.paris.fr/explore/dataset/arrondissements/).
...
```{r mutate2, warning=FALSE,message=FALSE,error=FALSE}
iris <- df %>% 
  filter(DEP=="75") %>%
  select(IRIS, COM, P13_POP, P13_POP0014:P13_POP75P) %>%
...
2. Choose a French city aside from Paris and create a map of the percentage of qualified professionals (cadre) by IRIS in that city (C13_POP15P_CS3)
